...
Solver::Solver( int N ) : _N(N) {
	...
	int i;
	for( i=0; i<SIZE ; i++ ) {
		_d[i]=0.0f;_d0[i]=0.0f;_T[i]=0.0f;_T0[i]=0.0f;
...
void advect_cool ( int N, int b, float * d, float * d0, float *T, float *T0,
		   float * u, float * v, float *w, float cool, float dt )
{
	int i, j, k, i0, j0, k0, i1, j1, k1;
	float x, y, z, s0, t0, r0, s1, t1, r1, dt0;
	float rhs, lhs, c0 ;
...
void Solver::densitiesStepWithTemp ( float diff, float cool, float dt )
{	
	addSource ( _N, _d, _srcd, dt );
	SWAP ( _d0, _d ); diffuse ( _N, 0, _d, _d0, diff, dt );
	SWAP ( _d0, _d ); advect_cool ( _N, 0, _d, _d0, _T, _T0, _u, _v, _w, cool, dt );
}
...
void Solver::velocitiesStepWithTemp ( float visc, float buoy, float dt )
{
	// Adding sources
	addSource ( _N, _u, _srcu, dt ); 
	addSource ( _N, _v, _srcv, dt ); 
