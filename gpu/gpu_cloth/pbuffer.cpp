...
PBuffer::PBuffer(int width,int height,int flags) : width(width), height(height)
{
    ...
    if(flags & RGB || flags & RGBA) {
        ...
        attrib.push_back(flags & FLOAT32 ? 32 : (flags & FLOAT16 ? 16 : 8) );
    ...
    if((flags & FLOAT32) || (flags & FLOAT16)) {
    ...
    try {
        ...
        //printf("PBuffer: using %d Kb of precious vidmem\n",
          //      (width*height*(flags & RGBA ? 4 : 3)*
            //            (flags & FLOAT32 ? 4 : (flags & FLOAT16 ? 2 : 1)) ) /1024 );
#if 1
